...
/**
 * A "Fetch Modus" is a module that tells the sylladex how to manage it's Card(s). <br/> This interface provides the
 * bare minimum required for proper manipulation of Card(s) with respect to the Sylladex <br/> <br/> A Modus needs to be
 * able to perform Card manipulation for item storage and retrieval. In a sense, a Fetch Modus is a controller that is
 * associated with a very specific model. Said model is simply stored in said controller for ease of modularity. <br/>
...
public interface Modus {
    ...
    /**
     * The {@link Metadata} object associated with this modus. Contains the command map, reference to this modus object,
     * and modus class name.
     *
     * @return the METADATA
     */
    ...
    /**
     * Save the current state of the modus. <br> Saving the modus should create a List deck of Card to be given to the
